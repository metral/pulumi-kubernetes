...
export async function checkCrdStatus(name: string, client: k8sClient.ApiextensionsV1Api): Promise<boolean> {
    for (let i = 0; i < 20; i++){
        try {
            const crd = await client.readCustomResourceDefinition(name);
            let conditions = crd?.body?.status?.conditions;
            if (conditions) {
                for (let c of conditions) {
                    if (c.type == "Established" && c.status == "True") {
                        console.log(`${name} CRD is ready`);
                        return true;
                    }
                }
            }
            ...
            console.log(`Waiting for ${name} CRD to be ready (${i+1})`)
